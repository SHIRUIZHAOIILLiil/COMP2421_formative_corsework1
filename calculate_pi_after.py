...
if __name__ == '__main__':
    ...
    with open("data_pi_Im.csv", 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Loop', 'Sides', 'pi', 'A_error', 'R_error'])

    for i in range(0, 51):
        outcome = calculate_pi_achi_im(i)
        ...
        with open("data_pi_IM.csv", 'a', newline='') as file:
            writer = csv.writer(file)
